...
2. Бот проверяет текст модерацией (см. ниже) и разбирает его: тип работы, число людей,
   цена, город, адрес, дата/время, телефон, `@username`. Разбор best-effort — что не
   распозналось, остаётся пустым.
3. Автор видит предпросмотр: свой текст без изменений + что бот из него понял.
   Кнопки «Опубликовать» / «Отменить». Чтобы поправить — просто прислать текст заново.
4. После публикации в канал уходит **исходный текст автора** (в этом смысл свободной
   формы), а разобранные поля наполняют ленту и поиск в приложении.
5. Автору приходит кнопка «Закрыть вакансию» — карточка в канале помечается закрытой.

Команды бота: `/start` — приветствие, `/help` — как писать объявление, `/cancel` — удалить черновик.
